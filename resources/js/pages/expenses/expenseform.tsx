...
//   const { toast } = useToast()
  ...
  // Initialize form when the component mounts or expense changes
  useEffect(() => {
    if (open) {
      // Reset the form with the expense data or default values
      const formData = {
        id: expense?.id || 0,
      ...
      // Initialize line items for the selected bucket
      if (expense?.bucket_id) {
        ...
        if (bucket && bucket.lineItems && bucket.lineItems.length > 0) {
          console.log("Setting initial line items:", bucket.lineItems)
          setLineItems(bucket.lineItems)
        }
      }
    }
  }, [expense, open, buckets])
